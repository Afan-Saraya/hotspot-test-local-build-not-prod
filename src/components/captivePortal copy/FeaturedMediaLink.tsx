...
export function FeaturedMediaLink({ item, language, title, subtitle }: FeaturedMediaLinkProps) {
  ...
  return (
    <section className="px-6 mt-0 mb-12">
      <a
        href={item.link}
        className="block group rounded-2xl overflow-hidden ring-1 ring-white/10 bg-white/5 hover:bg-white/10 transition-colors"
      >
        <div className="aspect-video w-full bg-black/30 rounded-2xl overflow-hidden">
          {item.imageFile && (
            <img
              src={item.imageFile}
              alt={sectionTitle}
